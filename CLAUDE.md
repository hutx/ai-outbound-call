...
- **LLMService**（`llm_service.py`）：LLM 服务（Anthropic SDK，支持流式/非流式，Dashscope 兼容 API）
...
- **AsyncScriptUtils**（`async_script_utils.py`）：话术脚本提示词构建（输出格式、客户信息、打断策略）
...
- **流程**：
  ...
  11. **LLM 流式输出** → 按句号切句 → 逐句 TTS 播报 → 解析 `<决策>` 标签获取 intent/action
  12. 循环对话
- **关键**：forkzstream WS 使用 callId 作为通话 UUID 标识
- **关键**：TTS 输出 16kHz PCM，`play_stream` 中 `_downsample_16k_to_8k` 降采样后发送
- **关键**：`play_stream` 流式发送 — TTS chunk 合并为 320 字节/帧（20ms @ 8kHz 16bit）后发送，确保 FS 端播放节奏稳定；返回 `sent_bytes` 用于计算精确播放时长
- **关键**：挂断语等待时长 = `sent_bytes / 16000 * 1000 + 2000ms`（基于实际发送字节数，非文本长度估算）
...
- **barge-in 噪声过滤**（2026-04-20 更新）：
  - **之前**：只丢弃纯英文（"ok", "system"），不丢弃中文语气词。导致用户听电话时的"嗯。"误触发打断，TTS 停止，LLM 内容被丢弃。
...
- **应用路径**：
  ...
  - barge-in 路径：`_barge_in_asr_loop_with_queue` 使用 **更新版过滤**（丢弃纯英文 + 单字语气词 + 重复语气词）
...
- `_conversation_loop` 中 `is_all_silent=True` 时不计入无回应，防止误挂断
- **快速超时机制**（2026-04-21）：`_listen_user()` 新增 `quick_timeout` 参数 — 在追问后使用，VAD 在该时间内未检测到语音（`_started_speaking=False`）则提前退出，返回 `("", True, True)` 表示无回应。避免追问后空等 15s ASR_TIMEOUT。
- **代码位置**：`backend/core/call_agent.py` — `AudioStreamAdapter._started_speaking`, `_listen_user(quick_timeout=...)`
- **日期**：2026-04-18

### ✅ 百炼 TTS 采样率
- TTS 输出格式：`PCM_16000HZ_MONO_16BIT`（提升音质）
- `play_stream` 中 `_downsample_16k_to_8k` 降采样为 8kHz 后发给 forkzstream/FreeSWITCH
- **帧缓冲发送**：不定长 TTS chunk 合并为 320 字节/帧（20ms）再发送，确保播放节奏稳定，避免一卡一卡
- **返回 sent_bytes**：`play_stream` 返回实际发送字节数，用于精确计算播放时长（`sent_bytes / 16000 * 1000`）
- **日期**：2026-04-18
...
- **`_conversation_loop()`**：先发 → 立即启动 LLM **流式**任务；后改 → 并行启动 `_listen_tolerance()` 宽容期监听
...
### ✅ LLM 流式输出 → TTS 流式播放（2026-04-20）
- **架构**：ASR → LLM(流式 token) → 按句号切句 → 每句 → TTS 流式 → 立即播放 → 解析 JSON intent/action
- **输出格式**（`async_script_utils.py` — `_build_output_contract()`）：
  1. 先输出纯文本回复内容（不含 markdown）
  2. 回复完成后，换行输出 `<决策>` 标签
  3. 标签内为 JSON，包含 `intent`、`action`、`action_params`（不再有 `reply` 字段）
- **流式实现**（`llm_service.py` — `chat_stream()`）：
  - 使用 Anthropic SDK `messages.create(..., stream=True)` 遍历事件
  - 提取 `content_block_delta` 事件的 `text_delta`
  - 检测 `<决策>` 标记停止 yield，解析标签内 JSON
  - **超时保护**：30s 超时，降级到非流式 `chat()` 调用
  - **Dashscope 兼容**：不使用 `messages.stream()` 上下文管理器（不兼容），直接遍历事件流
- **CallAgent 流式处理**（`call_agent.py` — `_think_and_reply_stream()`）：
  - 调用 `llm.chat_stream()` 逐 token 接收
  - 用 buffer 累积 token，遇到 `。` `！` `？` 时切出一句
  - 每句调 `_say()` 播放（复用现有 TTS 流式 + barge-in 逻辑）
  - `_say()` 返回 barge-in 信号时立即停止后续句子
  - 返回 `("", action)` 标记已逐句播报，`_conversation_loop` 不再二次播报
- **保留旧方法**：`_think_and_reply()`（非流式）作为降级方案保留
- **效果**：首句 TTS 开始时间从 5-15s 降到 2-4s
...
### ✅ 无回应挂断机制（2026-04-20）
- **判定条件**：VAD 全程静音（`is_all_silent=True`）不计入无回应；ASR 超时或识别到空文本算一次无回应
- **计数模式**：
  - `consecutive`（连续）：用户有效回应后计数器清零
  - `cumulative`（累计）：整个通话生命周期累计计数，不清零
- **达到阈值**：播放挂断语后结束通话。挂断语优先级：自定义挂断语 → 结束语（`closing_script`）→ "感谢接听，再见！"
- **配置字段**：话术表新增 `no_response_timeout`、`no_response_mode`、`no_response_max_count`、`no_response_hangup_msg`、`no_response_hangup_enabled`
- **追问语**：未达阈值时播报 "您好，请问您还在吗？"
- **`no_response_timeout` 字段**：当前为预留字段，存储但未接入实际 ASR 超时控制（仍使用硬编码 `ASR_TIMEOUT = 15.0`）
- **代码位置**：
  - `backend/core/call_agent.py` — `_conversation_loop()` 无回应计数逻辑
  - `backend/services/async_script_utils.py` — `get_no_response_config()`
  - `backend/models/call_script.py` — 数据库模型
  - `backend/api/scripts_api.py` — API 接收/返回
  - `backend/scripts/migrate_add_no_response_fields.py` — 数据库迁移
...
- `backend/` - Python FastAPI 后端
  ...
  - `services/` - 外部服务
    ...
    - `llm_service.py` — LLM 服务（Anthropic SDK，流式/非流式）
    - `esl_service.py` — ESL Inbound 连接池
    - `async_script_utils.py` — 话术脚本提示词构建 + 无回应配置读取
    - `script_service.py` — 话术脚本管理服务（CRUD + 缓存）
  - `scripts/` — 数据库迁移脚本
