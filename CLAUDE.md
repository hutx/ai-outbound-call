...
- **应用路径**：
  ...
  - barge-in 路径：`_barge_in_asr_loop_with_queue`、`_barge_in_asr_loop` 也需噪声词过滤

- **代码位置**：`backend/core/call_agent.py` — `_listen_user()` 内 `_is_noise_word()` 函数 + `adapter._max_rms < 1500`
